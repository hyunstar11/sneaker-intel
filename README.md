...
- **Software engineering practices**: Type-checked configuration with Pydantic, abstract base classes for extensibility, 41 unit tests across 14 modules, linting with ruff, dependency management with uv
...
The FastAPI service starts with `make run-api`. If a pre-trained model artifact exists (`models/ensemble_v1.joblib`), it loads instantly. Otherwise, it falls back to training at startup (~35s). Run `make train` to generate the artifact. Auto-generates OpenAPI docs at `http://localhost:8000/docs`.
...
│   ├── training.py                         # Train pipeline: saves model artifact to models/
│   │
│   ├── data/
...
└── tests/
    ...
    │   ├── test_size.py                    # Size normalization train/inference consistency
    │   └── test_stockx.py                  # Number of Sales reference and lookup
    ├── test_models/
...
make test             # 41 tests, all should pass
make lint             # ruff check on src/ and tests/

# Train the model (saves artifact to models/ensemble_v1.joblib)
make train

# Run the API (loads artifact instantly, or falls back to training)
make run-api
...
41 tests across 14 test modules:
...
| `test_features/test_stockx.py` | 2 | Number of Sales reference building and lookup |
